...
int adjacent(char* board, int offset, int width, int height){//checks the state of surrounding cells
  ...
  char* cell = board + offset;
  for (int i = offset - width - 1; i < offset - width + 1; i++){//cells to north
    if (*(board + i) == ALIVE){
      alive++;
    }
  }
  for (int i = offset + width - 1; i < offset + width + 1; i++){//cells to south
    if (*(board + i) == ALIVE){
      alive++;
    }
  }
...
char* advance(char* board, int width, int height){
  ...
  for (int i = 0; i < size; i++){
    ...
    newCell = refresh + i;
    bool edge = false;
    edge = isEdge(board, i, width, height) != 0;
    if (!edge){
